/*	This file is part of the software similarity tester SIM.
	...
	$Id: hash.c,v 2.51 2020-08-14 16:56:50 dick Exp $
*/
...
static void init_hash_table(void) {
	...
	&&	/* and there is still a (prime) size left to try */
		...
		latest_index = (size_t *)
			Try_Calloc(latest_index_table_size, sizeof (size_t));
		n--;
	}
	if (!latest_index) {
...
#define	SHIFT	(5)
		...
		for (j = txt->tx_start;	j < txt->tx_limit; j++) {
			if (	/* we have a complete hash value */
				...
				int oldest_value =
				    Token_To_Int(Token_Array[j - Min_Run_Size]);
				int oldest_shift =
			...
			/* Add new token */
			hash ^= Token_To_Int(Token_Array[j]);
			...
			/* If have we assembled a complete hash value now,
			   ...
			   so we code instead:
			*/
			if (j - txt->tx_start < (size_t)(Min_Run_Size - 1)) {
				/* no */
				continue;
			}
...
void Make_Forward_References(void) {
...
void Free_Forward_References(void) {
