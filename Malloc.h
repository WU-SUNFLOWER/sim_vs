/*	This file is part of the checking memory manager MALLOC.
    ...
    $Id: Malloc.h,v 1.20 2020-08-14 16:56:47 dick Exp $
*/
...
2.  void *Try_Malloc(size_t s)
    void *Try_Calloc(size_t n, size_t s)
    void *Try_Realloc(void *p, size_t s)
    void Out_Of_Memory_Exit(const char *msg)
    ...
    A call of Out_Of_Memory_Exit(msg) gives an out-of-memory error message,
...
4.  void Report_Memory_Status(FILE *f)

    Produces a compacted list of allocated but not yet freed blocks on the
    stream f, with information about file name and line number where they were
    allocated. Allocated and later reallocated blocks that were not freed are
    reported where they were last reallocated.
    This information is useful to get insight into memory use and abuse.

5.  void Mem_Clobber(void *p, size_t size)
    ...
    The routine that performs the clobbering, Mem_Clobber(void *p, size_t size),
...
#define	Try_Malloc(s)	(_mreg_malloc(0, (s), __FILE__, __LINE__))
#define	Try_Calloc(n,s)	(_mreg_calloc(0, (n), (s), __FILE__, __LINE__))
#define	Try_Realloc(p,s)(_mreg_realloc(0, (void *)(p), (s), __FILE__, __LINE__))
#define	Out_Of_Memory_Exit(s) _out_of_memory((s), __FILE__, __LINE__, 0)

#define	new(type)	((type *)Malloc(sizeof (type)))
#define	try_new(type)	((type *)Try_Malloc(sizeof (type)))
#define	new_string(s)	(_new_string(1, (s), __FILE__, __LINE__))
#define	try_new_string(s)(_new_string(0, (s), __FILE__, __LINE__))

extern void Report_Memory_Status(FILE* f);
extern void Mem_Clobber(void* p, size_t size);
