/*	This file is part of the auxiliaries library.
	...
	$Id: fname.c,v 1.16 2020-11-06 13:58:12 dick Exp $
*/
...
/*	Two data types are involved in Unicode file names in Windows:
	...
	The conversion between these two proceeds through code points,
	...
	The module consists of two levels: a set of static routines
		...
		int UTF8_sequence_to_Code_Point(const UTF8 *s, UTF32 *cp),
		int UTF16_sequence_to_Code_Point(const UTF16 *s, UTF32 *cp),
		const UTF8 *Code_Point_to_UTF8_sequence(UTF32 c),
		const char *Code_Point_to_UTF16_sequence(UTF32 cp)

	which convert from and to Code_Points; and two global routines

		const char *Fname2str(const char *fn),
		const char *str2Fname(const char *s)
...
get_UTF8_tail(const UTF8* s, int n) {
	...
	/* scoop up n UTF-8s */
	for (i = 0; i < n; i++) {
		if ((s[i] & LMASK(2)) != LMASK(1)) return BAD_Code_Point;
...
static int /* number of UTF8s used; cp = BAD_Code_Point for error */
UTF8_sequence_to_Code_Point(const UTF8* s, UTF32* cp) {
	UTF8 head = s[0];
	int head_length = 1;
	const UTF8* tail = &s[1];
	int tail_length;
	...
	if (tail_value == BAD_Code_Point) goto error;
...
static int /* number of UTF16s used; cp = BAD_Code_Point for error */
UTF16_sequence_to_Code_Point(const UTF16* s, UTF32* cp) {
...
Code_Point_to_UTF8_sequence(UTF32 c) {
...
Code_Point_to_UTF16_sequence(UTF32 cp) {
	/* adapted from code from http://unicode.org/faq/utf_bom.html */
	static UTF16 res[3];
	...
	/* the code point does not correspond to a UTF16 sequence */
	return NULL;
}

const char* Fname2str(const char* fn) {
	/* converts a char (wchar_t) string to an UTF-8 string */
	static UTF8 res[MAX_PATH_NAME_LENGTH];
	UTF8* rp = &res[0];
	int i = 0;
	...
	while (fn[i]) {
		...
		/* get code point from one or two char chars */
		i += UTF16_sequence_to_Code_Point(&fn[i], &cp);
		if (cp == BAD_Code_Point) break;

		/* convert code point to UTF8 sequence */
		p = Code_Point_to_UTF8_sequence(cp);
		if (p == NULL) break;
...
const char* str2Fname(const char* s) {
	/* converts a possibly UTF-8 string to an char (wchar_t) string */
	static char res[MAX_PATH_NAME_LENGTH];
	char* rp = &res[0];
	int i = 0;
	...
	while (s[i]) {
		...
		/* get code point from one to four UTF-8s */
		i += UTF8_sequence_to_Code_Point((const UTF8*)&s[i], &cp);
		if (cp == BAD_Code_Point) break;

		/* convert code point to UTF-16 sequence */
		p = Code_Point_to_UTF16_sequence(cp);
		if (p == NULL) break;
...
static void
satisfy_lint(void* x) {
	/* lint cannot handle char complications */
	satisfy_lint(x);
}
#endif	/* lint */
