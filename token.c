...
void fprint_Token(FILE* ofile, const Token tk) {
	...
	*/
	int tki = Token_To_Int(tk);
	int ch = tki & 0x7F;  // tki & 0b0111_1111
	int bit8 = tki & 0x80;  // bit8 & 0b1000_0000


	if (Token_EQ(tk, No_Token)) { 
		fprintf(ofile, "--"); 
		return; 
	}
	if (Token_EQ(tk, IDF)) { 
...
void fprint_Tokens(const char* filename, const Token* src, const size_t length) {
	FILE* ofile = fopen(filename, "w+");
	for (size_t i = 0; i < length; ++i) {
		fprint_Token(ofile, src[i]);
	}
	fclose(ofile);
}

const char* size_t2string(size_t s) {
